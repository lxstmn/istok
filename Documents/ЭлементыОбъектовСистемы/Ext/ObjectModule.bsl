...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеКолонки)
	Если Не ОбъектСистемы.ВестиВариантыОбъектов Тогда
		ПроверяемыеКолонки.Удалить(ПроверяемыеКолонки.Найти("ВариантОбъекта"));
	КонецЕсли;
КонецПроцедуры
...
Процедура ПриЗаписи(Отказ)
			...
			Для Каждого СтрокаТаблицаОбъекта Из ТаблицаОбъекта.Значение Цикл
				...
				Для Каждого СтрокаКолонка Из ТаблицаОбъекта.Ключ.Колонки Цикл
					...
					Если СтрокаТаблицыСсылка = Неопределено Тогда
						Если ЗначениеЗаполнено(СтрокаТаблицаОбъекта.СтрокаТаблицы) Тогда
	...
	Запрос.УстановитьПараметр("Источник", Ссылка);
	Запрос.УстановитьПараметр("Автор", Автор);
	Запрос.Текст = ПолучитьЗапросИзменениеЗначенийКолонокОбъектов();
...
Функция ПолучитьЗапросИзменениеЗначенийКолонокОбъектов()
	Возврат
	...
	|	ЕСТЬNULL(МАКСИМУМ(ВТ_Итог.ЗначениеСтарое), &УникальныйИдентификатор) <> ЕСТЬNULL(МАКСИМУМ(ВТ_Итог.ЗначениеНовое), &УникальныйИдентификатор)";
КонецФункции
